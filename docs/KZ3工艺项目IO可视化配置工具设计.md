...
- 在线会话通过分页接口采样并直接展示全部北向点位，不再要求逐点加入关注组。分页尚未完成、请求失败或
  没有有效样本时显示 `— / NO DATA`，不得解释为 0 或 OFF。
...
| 能力 | 当前实现 | 明确边界 |
| --- | --- | --- |
| 本地 HTTP 代理 | Rust/Tauri 后端只允许固定诊断资源、`/api/v1/points` 分页 GET、`/api/v1/point/<name>` GET 和严格形状的 POST | 禁止任意 URL、任意路径、URL 凭据、`point/state` 直接写入；仅允许 HTTP 根地址 |
...
| 点位监视 | 从当前北向映射生成本地 descriptor，连接后自动分页轮询并直接显示全部北向值 | 分页值不提供 schema；设备质量与工具本地陈旧分别显示，搜索和分类只影响视图，不影响采样 |
...
诊断资源，并从 `offset=0&limit=30` 开始跟随响应 `next_offset` 读取全部北向值。2048 B 响应体可能使
实际 `count` 小于 30，客户端不得自行固定加 30。页面关闭后停止轮询。断开或重连后，
旧会话迟到响应必须丢弃，不能污染新会话状态。

在线调试布局以 PLC Watch Table 为主体：目标身份固定显示，实时点表占据首屏，质量和时效紧邻实时值，
操作列冻结。北向字段按“控制命令、可调参数、状态反馈、过程 I/O、其他上报”固定分组，分组头
同时显示字段数和可写/只读属性，并支持独立或整体展开、收起；控制与调参不得和平铺的只读
...
RS-485 设备切换并提供点位筛选，所有具备北向描述符的 Southbound 点直接显示主会话分页采样值。
上百点第三方产品只在浮窗内部滚动，不扩大主调试画布。

浮窗必须复用在线调试 Store 中的会话、诊断快照和样本，禁止自行连接设备或启动第二套轮询。
