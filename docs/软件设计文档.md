...
- **指令协议详情**：
  - 获取配置：发送 `SLE:LIST\r\n`。响应为 `SLE_CFG: NETNAME=MyNet, APID=12, PWR=5, MAXPWR=8` 等键值对。
  - 修改配置：
    - `SLE_NETNAME:<name>\r\n`（1-16字符ASCII）
    - `SLE_APID:<0-255>\r\n`
    - `SLE_PWR:<1-8>`（严禁\r\n，9字节定长）
...
- **数据流说明**：UI 加载时触发批量读取配置，展示数据一致性。更新时单项下发，等待对应 `OK` 响应或超时回调。
...
- **UI wireframe描述**：
  ...
  - **Data Grid 表格**：最大支持 16 行，支持行内点击直接编辑数字（如点击从站单元格即变成输入框），类型和功能码为下拉菜单。
  - **操作列**：包含拖拽调整顺序的手柄图标、单行调试按钮（弹出抽屉显示该点位的单次读取十六进制及解析结果）、删除按钮。

- **数据流说明**：前端表格维护一份完整数组，增删改只修改本地状态。点击“批量下发”时，由前端组合完整拼接字符串传给 Rust 后端，Rust 层包裹指令头 `RS485DEV:` 并附加 `\r\n` 送到串口执行，等待返回 `OK: UPDATE` 视为成功。
...
| SLE_NETNAME | `SLE_NETNAME:<name>` | 是 | 1-16 字符 | 标准 AT 风格截断 |
