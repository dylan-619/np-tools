...
> 软件版本：NP-Tools 1.0.4；HTTP API：KZ3 v1；文档更新日期：2026-09-16。
...
- 通过 `/api/v1/points?offset=&limit=30` 自动翻页读取并直接显示全部北向点位，无需逐点打开或加入监视组；
...
4. 工具会自动分页采样并显示全部业务北向点；需要缩小范围时直接使用名称搜索、类型分类或拓扑筛选。检查质量码、变化时间、健康项和 active fault。
...
- `/api/v1/points` 已提供分页实时值，但尚不提供设备侧 schema 元数据、跨页原子快照或采样 revision；工具仍以已核对的本地点表解释类型与权限。
